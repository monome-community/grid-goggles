/**
 * OscP5 is a processing and java library for the open sound control protocol,
...
public class OscP5 implements UdpPacketListener, TcpPacketListener {
	...
	/*
	 * @TODO
	 * implement polling option to avoid threading and synchronization issues.
	 * check email from tom lieber. look into mutex objects.
     ...
     * @invisible
     */
    public static final String NETVERSION = "0.9.5";
    ...
    static {
        if (!NETVERSION.equals(NetP5.VERSION)) {
      System.out.println("### INFO netP5 and oscP5 dont have the same version number.\n" +
                         "### please update both libraries at http://www.sojamo.de/libraries");
        }
    }
    ...
    public OscP5(
        ...
        if (_myOscProperties.networkProtocol() == OscProperties.MULTICAST) {
            Logger.printInfo("OscP5", "is joining a multicast group @ " + _myOscProperties.remoteAddress().address() + ":"
                             + _myOscProperties.remoteAddress().port());
        } else {
            Logger.printInfo("OscP5", "is running. you (" + ip() + ") are listening @ port "
                             + _myOscProperties.remoteAddress().port());
        }
    }
    ...
    public OscP5(
        ...
        case (TCP):
            ...
            break;
        case (MULTICAST):
            Logger.printWarning("OscP5", "please specify a multicast address. use "
    ...
    private void welcome() {
        System.out.println("OscP5 " + VERSION + " " +
...
//            return;
    ...
    /**
     * you can send osc packets in many different ways. see below and use
     * the send method that fits your needs.
     *
     *
