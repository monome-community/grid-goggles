...
public class GLTexture extends PImage implements PConstants, GLConstants {
  ...
  protected Method disposeBufferMethod = null;
  protected Object diposeBufferHandler = null;
  protected LinkedList<BufferData> bufferList = null;  
  ...
  public void delete() {
    ...
    if (bufferList != null && 0 < bufferList.size() && disposeBufferMethod != null) {
      while (0 < bufferList.size()) {
        BufferData data = bufferList.remove(0);
        data.dispose();
      }
    }
  }
  ...
  public void setPixelSource(Object obj) {
    diposeBufferHandler = obj;
    try {
      disposeBufferMethod = obj.getClass().getMethod("disposeBuffer", new Class[] { Object.class});
    } catch (Exception e) {
      e.printStackTrace();
    }    
  }
  ...
  public void copyBuffer(Object natBuf, IntBuffer rgbBuf, int w, int h) {
    if (bufferList == null) {
      bufferList = new LinkedList<BufferData>();
    }
    bufferList.add(new BufferData(natBuf, rgbBuf, w, h));
  }    
  ...
  protected class BufferData {
    ...
    void dispose() {
      try {
        disposeBufferMethod.invoke(diposeBufferHandler, new Object[] { natBuf });
      } catch (Exception e) {
        e.printStackTrace();
      }      
    }
