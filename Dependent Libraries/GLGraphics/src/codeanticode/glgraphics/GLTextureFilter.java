...
public class GLTextureFilter implements GLConstants, PConstants {
  ...
  protected void addParameter(XMLElement xml) {
    ...
    if ((-1 < parType) && !paramsHashMap.containsKey(parName)) {
      param = new GLSLTextureFilterParameter(parent, parName, parLabelStr, parType);
      param.setValue(parValue);
      paramsHashMap.put(parName, param);
      paramsArray = (GLTextureFilterParameter[]) PApplet.append(paramsArray,
          param);
    }
  }
  ...
  protected void initShader(String xmlFilename, boolean useURL) {
    ...
    for (int i = 0; i < numInputTex; i++) {
      srcTexUnitParams[i] = new GLSLTextureFilterParameter(parent, srcTexNames[i], "Texture unit " + i,  SHADER_VAR_INT);
      srcTexUnitParams[i].setShader(shader);
      srcTexUnitParams[i].init();
      srcTexOffsetParams[i] = new GLSLTextureFilterParameter(parent, srcTexOffsetNames[i], "Texture offset " + i,  SHADER_VAR_VEC2);
      srcTexOffsetParams[i].setShader(shader);
      srcTexOffsetParams[i].init();      
    }
    clockDataParam = new GLSLTextureFilterParameter(parent, clockDataName, "Clock data",  SHADER_VAR_VEC2);
    destColorParam = new GLSLTextureFilterParameter(parent, destColorName, "Destination color",  SHADER_VAR_VEC4);
    destTexSizeParam = new GLSLTextureFilterParameter(parent, destTexSizeName, "Destination size",  SHADER_VAR_VEC2);
